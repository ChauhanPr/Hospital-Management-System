...
$p_mode=$_POST['p_mode'];
 ...
 $query = "INSERT INTO payment (pid,p_id,p_name,p_date,p_mode,p_amt) VALUES ('$pid','$p_id','$p_name','$p_date','$p_mode','$p_amt')";
 $result = pg_query($dbconn, $query)or die("Query failed");

 if (!$result) {
   echo "Error: " . pg_last_error($dbconn);
   } else {
	echo "<h1><center>Payment Done successfully!</center></h1>";
